...
party = {'BJP': [], 'Congress': [], 'AAP': [], 'Others': [],'TMC' : []}
...
class Voting_Blockchain(object):
    ...
    def create_vote(self):
        try:
            ...
            seller = int(input("\nEnter the Voter ID: "))
            buyer = input("Enter the Pary Name: ")
            # pid = int(input("Enter the Property ID: "))
            if (not self.verify_transaction(seller, buyer)):
            ...
            self.users[seller]['Voted'] = True
            self.users[seller]['Party Voted'] = buyer
            party[buyer].append(seller)
    ...
    def verify_transaction(self, seller, buyer):
        if (buyer not in party):
            print("You cannot vote a party which does not exist")
            return False
        if(self.users[seller]['Voted'] == True):
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if (choice == '1'):
            ...
            print("Hope you had a blast Voting!!")
            break
        else:
